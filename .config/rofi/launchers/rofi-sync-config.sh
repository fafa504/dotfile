...
SOURCE_DIR="$HOME"
REMOTE_NAME="Drive:"
DEST_PATH="[5] Archive/Config/Linux/"
FILTER_FILE="$HOME/.config/rclone/filter/config.txt"
...
rclone_params=(
    ...
    "--filter-from=$FILTER_FILE" # FIle lọc ra những đối tượng cần đồng bộ
)

# --- Bắt đầu Script ---
date_start=$(date +"%d.%m.%y-%H:%M:%S")
echo "[$date_start] Bắt đầu đồng bộ..." >>"$LOG_FILE"
notify-send "Bắt đầu đồng bộ Config..." "Ghi log tại: $LOG_FILE"
konsave --save "tahoe-blur" -f >>"$LOG_FILE"
notify-send "Khởi tạo konsave thành công ..."
