...
function VolumeSlider:render()
	...
	local vol = state.volume or 50 -- mặc định 50%
	local vol_max = state.volume_max or 100 -- mặc định 100%

	local volume_y = self.ay + self.border_size + ((height - (self.border_size * 2)) * (1 - math.min(vol / vol_max, 1)))
